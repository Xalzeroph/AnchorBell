...
impl SimulationEngine {
                    ...
                    let intent = if strategy_variant == SimulationPolicyVariant::M0Fixed {
                        ...
                        let input = threshold.map(|threshold| SignalInput {
                            ...
                            price_tick: self
                                .execution_filters
                                .get(symbol)
                                .map(|filters| filters.price_tick)
                                .unwrap_or(0),
                            bid_quantity: book.bid_quantity,
                            ask_quantity: book.ask_quantity,
                            buy_queue_ahead,
    ...
    fn place_symbol(
        ...
        // Post-only validity is defined by non-crossing the opposite touch.
        // A maker buy may improve the bid inside the spread, and a maker sell
        // may improve the ask inside the spread. Requiring buy <= best bid /
        // sell >= best ask would silently reject the queue-priority quotes
        // produced by SignalPolicy.
