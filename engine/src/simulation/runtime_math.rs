...
pub(super) fn dynamic_threshold_diagnostic_for(
    ...
    // Funding is a real cash-flow term, not a fixed clock surcharge. The
    // previous 60/30/10-minute buckets charged 10/25/50 bps regardless of the
    // observed rate and could double-count the M8/M9 funding controller. For
    // CoreV1, charge only the adverse funding carry of the current signal side;
    // M8/M9 already price it in their causal funding decision.
    let deadline_risk_pico_bps = if variant.uses_funding_controller() {
        0
    } else if state.next_funding_time_ms > timestamp_ms {
        let mid_price_ticks = (book.bid_price_ticks + book.ask_price_ticks) / 2;
        let signal_side = fair_value_for_state(state)
            .map(|estimate| {
                if estimate.price.0 > mid_price_ticks {
                    1_i128
                } else if estimate.price.0 < mid_price_ticks {
                    -1_i128
                } else {
                    0_i128
                }
            ...
            .unwrap_or(0);
        let adverse_funding_pico_bps = state
            .latest_funding_rate_e8
            .map(|rate| (signal_side * i128::from(rate) * i128::from(PICO_BPS_SCALE)) / 10_000_i128)
            .unwrap_or(0);
        adverse_funding_pico_bps.clamp(0, i128::from(i64::MAX)) as i64
    } else {
        0
    };
...
pub(super) fn m7_entry_admissible(state: &SimulationSymbolState, threshold_pico_bps: i64) -> bool {
    ...
    if book.bid_price_ticks <= 0
        || book.ask_price_ticks < book.bid_price_ticks
        || fair_value_ticks <= 0
        || threshold_pico_bps <= 0
    {
        return false;
    }
    let buy_edge_pico_bps = edge_pico_bps(fair_value_ticks, book.bid_price_ticks).unwrap_or(0);
    let sell_edge_pico_bps = edge_pico_bps(book.ask_price_ticks, fair_value_ticks).unwrap_or(0);
    let best_quote_edge_pico_bps = buy_edge_pico_bps.max(sell_edge_pico_bps);
    // M7 treats very large quote edges under elevated stress as repricing,
    // not a free mean-reversion edge, while preserving ordinary opportunities.
    best_quote_edge_pico_bps >= threshold_pico_bps
        && !(best_quote_edge_pico_bps >= 500 * PICO_BPS_SCALE
...
pub(super) fn update_markout_feedback(
    ...
    while let Some(observation) = state.pending_markouts.front().copied() {
        ...
        state.ewma_adverse_markout_pico_bps = (i128::from(state.ewma_adverse_markout_micro_bps)
            * i128::from(PICO_BPS_SCALE / MICRO_BPS_SCALE))
        .clamp(0, i128::from(i64::MAX)) as i64;
