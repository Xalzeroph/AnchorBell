...
function renderSimulationMetrics(data) {
  ...
  $("simulationRows").innerHTML = (data.symbols || []).map((x) => { const win = x.fills ? (Number(x.winning_fills || 0) / Number(x.fills) * 100).toFixed(1) + "%" : "—"; return "<tr><td>" + escapeHtml(x.symbol) + "<small>" + escapeHtml((x.calendar_state || "") + (x.data_quality ? " · " + x.data_quality : "")) + "</small></td><td>" + signed(x.net_pnl_ticks) + "</td><td>" + signed(x.market_pnl_ticks) + "</td><td>" + signed(x.strategy_pnl_ticks) + "</td><td>" + signed(x.funding_pnl_ticks) + "</td><td>" + signed(x.fees_ticks) + "</td><td>" + win + "</td><td>" + number(x.position) + "</td></tr>"; }).join("") || "<tr><td colspan='8'>尚未收到标的指标</td></tr>";
}
