...
replacement = r'''// The explicit legacy arguments remain as compatibility wrappers. New research
// and OOS validation should use ReplayConfig so the policy and execution
// assumptions are part of one auditable object.
#[derive(Debug, Clone)]
...
pub fn replay_jsonl_with_config(
    ...
    {
        return Err(SimulationError::InvalidConfig("invalid replay policy configuration"));
    }
    let allocations = config
        .capital_usdt_ticks
        .map(|capital| allocate_positions(&anchors, capital, &BTreeMap::new(), config.quantity_scale))
        .transpose()?;
...
fn parse_args() -> Result<Args, String> {
    ...
    while let Some(flag) = args.next() {
        match flag.as_str() {
            ...
            "--capital-usdt" => capital_usdt_ticks = Some(parse_usdt_ticks(&next(&mut args, &flag)?)?),
...
fn parse_usdt_ticks(value: &str) -> Result<i64, String> {
    ...
    let whole = whole.parse::<i64>().map_err(|_| "--capital-usdt is too large")?;
    let mut fraction = fraction.to_owned();
    while fraction.len() < 8 {
        fraction.push('0');
    }
    let fraction = fraction.parse::<i64>().map_err(|_| "invalid --capital-usdt")?;
    whole
...
print("policy-aware replay transformation complete")
